...
function Dashboard({data,setView,st}){
  const stats=[
    {l:'Protocolos Pendentes',v:data.protocolos.filter(p=>p.status==='recebido'||p.status==='em_analise').length,c:'#c9a84c',view:'protocolos'},
    {l:'Demandas Ativas',v:data.demandas.filter(d=>d.status!=='concluida').length,c:'#4a90d9',view:'demandas'},
    {l:'Judiciais Ativos',v:data.judiciais.filter(j=>j.fase!=='encerrado').length,c:'#9b59b6',view:'judiciais'},
    {l:'Contratos Vigentes',v:data.contratos.filter(c=>c.status==='vigente').length,c:'#4caf82',view:'contratos'},
    {l:'Pareceres Emitidos',v:data.pareceres.length,c:'#e09440',view:'pareceres'},
    {l:'Contratos Vencendo',v:data.contratos.filter(c=>{const dl=daysLeft(c.fim);return c.status==='vigente'&&dl!==null&&dl<=30}).length,c:'#e05c5c',view:'contratos'},
  ]
  return(
    <div>
      ...
      <div style={{padding:'22px 28px'}}>
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:14,marginBottom:22}}>
          {stats.map(s=><div key={s.l} onClick={()=>setView(s.view)} style={{background:'#1e2d40',border:'1px solid rgba(255,255,255,0.06)',borderRadius:10,padding:'16px 18px',borderTop:'3px solid '+s.c,cursor:'pointer'}}>
            <div style={{fontSize:10,color:'#8a9bb0',textTransform:'uppercase',letterSpacing:0.8,marginBottom:8}}>{s.l}</div>
        ...
        <div style={{background:'#1e2d40',border:'1px solid rgba(255,255,255,0.06)',borderRadius:10}}>
          <div style={{padding:'14px 18px',borderBottom:'1px solid rgba(255,255,255,0.06)',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
            <h3 style={{fontFamily:"'Playfair Display',serif",fontSize:14,margin:0}}>Protocolos Recentes</h3>
...
function Demandas({data,onUpdate,showNotify,st}){
  ...
  return(
    <div>
      <div style={st.topbar}><div><h2 style={st.h2}>Demandas Internas</h2></div><button onClick={()=>{setForm({});setModal('f')}} style={st.btnP}>+ Nova</button></div>
      <div style={{padding:'20px 28px'}}>
        <div style={{display:'flex',gap:0,borderBottom:'1px solid rgba(255,255,255,0.06)',marginBottom:18}}>
        ...
        <table style={{width:'100%',borderCollapse:'collapse'}}>
          ...
          <tbody>
            {dados.length===0?<tr><td colSpan={cfg.cols.length+1} style={{...st.td,textAlign:'center',padding:40,color:'#8a9bb0'}}>Nenhum registro</td></tr>:dados.map(row=>(
              <tr key={row.id}>
                {cfg.cols.map(c=><td key={c.k} style={st.td}><span style={{fontSize:12}}>{row[c.k]||'-'}</span></td>)}
                <td style={st.td}><div style={{display:'flex',gap:4}}>
